                                    ...
                                    <div class="card">
										...
										<div class="card-block table-border-style" style="margin-top: -15px">
                                			<div class="table-responsive">
                                    			<table class="table table-striped table-hover table-bordered nowrap dataTable no-footer">
													<thead>
														<tr>
														...
														<template v-for="(data,index) in varList">
															...
															<#list fieldList as var>
																...
																<#else>
																	...
																	<td>
																		<a v-show="cha" title="查看" v-on:click="goView(data.${objectNameUpper}_ID)" style="cursor:pointer;"><i class="feather icon-eye"></i></a>
																		<a v-show="edit" title="修改" v-on:click="goEdit(data.${objectNameUpper}_ID)" style="cursor:pointer;"><i class="feather icon-edit"></i></a>
									                 					<a v-show="del" title="删除" v-on:click="goDel(data.${objectNameUpper}_ID)" style="cursor:pointer;"><i class="feather icon-trash-2"></i></a>
																	</td>
																</tr>
														</template>
												...
												<table style="width:100%;margin-top:15px;">
													<tr>
														<td style="vertical-align:top;">
															<button v-show="add" class="btn btn-primary btn-sm" v-on:click="goAdd(${objectNameUpper}_ID)">新增</button>
...
var vm = new Vue({
	...
	methods: {
		...
		goEdit: function (${objectNameUpper}_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '../../${packageName}/${objectNameLower}/${objectNameLower}_edit.html?PARENT_ID='+this.${objectNameUpper}_ID+'&${objectNameUpper}_ID='+${objectNameUpper}_ID;
			 diag.Width = 1000;
			 diag.Height = 800;
			 diag.CancelEvent = function(){ //关闭事件
				 var varSon = diag.innerFrame.contentWindow.document.getElementById('showform');
		...
		//查看
		goView: function (${objectNameUpper}_ID){
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="查看";
			 diag.URL = '../../${packageName}/${objectNameLower}/${objectNameLower}_view.html?PARENT_ID='+this.${objectNameUpper}_ID+'&${objectNameUpper}_ID='+${objectNameUpper}_ID;
			 diag.Width = 1000;
			 diag.Height = 800;
			 diag.CancelEvent = function(){ //关闭事件
				 diag.close();
        ...
        getUrlKey: function (name) {
      	...
      	//判断按钮权限，用于是否显示按钮
        hasButton: function(){
        	var keys = '${objectNameLower}:add,${objectNameLower}:del,${objectNameLower}:edit,${objectNameLower}:cha,toExcel';
        	$.ajax({
        		xhrFields: {
                    withCredentials: true
                },
        		...
        		success: function(data){
        		 if("success" == data.result){
					...
					vm.cha = data.${objectNameLower}fhadmincha;
